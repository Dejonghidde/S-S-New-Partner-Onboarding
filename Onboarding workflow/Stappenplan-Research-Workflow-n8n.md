...
- [ ] **Stap 2.1:** Volg de n8n-MCP-werkvolgorde (zie randvoorwaarden). Nodes: Execute Workflow Trigger (input), HTTP Request (GET `https://{{domein}}`, user-agent `Mozilla/5.0 (compatible; SS-ResearchBot/1.0)`, timeout 30s, redirects volgen), Code-node laag 1 (laad de patroonbibliotheek als constante in de code; pas alle laag-1-patronen toe op de HTML), IF (GTM gevonden?), HTTP Request laag 2 (GET `https://www.googletagmanager.com/gtm.js?id={{GTM-id}}`), Code-node laag 2 (pas laag-2-patronen toe op de containerbody), Merge, Code-node output (bouw het output-object; dedupliceer per tool; zet `niet_scanbaar: true` als de eerste fetch een 403/blokkade gaf).
- [ ] **Stap 2.2:** `validate_workflow`, fix fouten, dan `create_workflow_from_code` met beschrijving "Tag-scan sub-workflow voor het Partner Research Pack (laag 1: HTML, laag 2: GTM-container). Inactief, handmatig."
- [ ] **Stap 2.3: Regressietest.** Draai de workflow met `test_workflow` (of `execute_workflow`) voor elk van de 5 testsites. Leg per site de gevonden set vast in `testset-tagscan.md` als baseline. Verwacht minimaal: het platform van elke site wordt herkend, en op minstens 3 van de 5 sites worden GTM of GA4 gevonden.
- [ ] **Stap 2.4:** LOG.md bijwerken (workflow-id, testresultaten per site) en commit.
...
- [ ] **Stap 4.1: Semrush-branch.** Eerst de verificatiecall (eenmalig): `GET https://api.semrush.com/?type=domain_ranks&key={{credential}}&domain={{domein}}&database=nl&export_columns=Dn,Rk,Or,Ot,Ad,At` (1 regel, ~10 units). Daarna in de workflow drie HTTP Request-nodes: domain_ranks (overview), `type=domain_organic&display_limit=50&export_columns=Ph,Po,Nq,Tr&database=nl` (top-keywords), `type=domain_organic_organic&display_limit=10&export_columns=Dn,Cr,Np,Or&database=nl` (organische concurrenten). Parse de CSV-respons in een Code-node. Log het unit-verbruik.
- [ ] **Stap 4.2: Ad-libraries-branch.** Voorfilter: alleen platforms checken waarvan de tagscan een pixel vond, plus altijd Meta en Google. Meta en Google via Apify-actors: zoek in de Apify Store (via de Apify-API of de store-pagina) een actor voor "Meta Ad Library" en een voor "Google Ads Transparency", kies op aantal gebruikers en recente updates, en test elke actor eerst los met één bekend adverterend merk voordat je hem in de workflow zet. TikTok alleen als er een TikTok-pixel is: zelfde aanpak (Commercial Content Library-actor). LinkedIn: sla programmatische check over; zet in het pack een directe link naar `https://www.linkedin.com/ad-library/search?companyIds=` met de instructie voor de reviewer om dit handmatig te checken (geen API beschikbaar). Resultaat per platform: `{"actief": true/false/"onbekend", "aantal_ads": n, "voorbeelden": [urls]}`.
- [ ] **Stap 4.3: Reviews-branch.** Open Make-scenario 3033093 ("Review Scraper | Trustpilot + Google") via de Make-MCP-tools (alleen lezen) en noteer welke Apify-actors daar gebruikt worden; hergebruik diezelfde actor-id's in n8n via de Apify-credential. Haal op: Google-rating plus aantal, Trustpilot-score plus aantal, en de 20 meest recente reviews. Thema-extractie: één Gemini Flash-call ("Cluster deze reviews in maximaal 5 terugkerende thema's, gesplitst in positief en negatief, elk met één letterlijk citaat.").

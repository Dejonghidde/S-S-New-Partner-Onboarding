...
- [x] **Stap 4.4: LinkedIn-bedrijfsdata.** Gebouwd: `automation-lab/linkedin-company-scraper` (actorId `kH7li2wTed8S3VaiV`) gevonden via Apify's store-API, HTTP-node plus parse-Code-node toegevoegd, resultaat onder sleutel `linkedin_company`. Aangesloten op de profiel-agent-prompt en de aannames-sectie (voorheen hardcoded "niet gebouwd"). Getest met realistische pin-data (execution 97079): sector, medewerkersaantal, locatie en vacatures komen correct door in de echte, live profiel-agent-output. Zie LOG.md "Taak 4.4".
- [x] **Stap 4.5: Socials-branch.** Alleen Instagram gebouwd deze sessie (`apify/instagram-profile-scraper`, actorId `dSCLg0C3YEZ83HzYX`); TikTok/YouTube/X/LinkedIn-profielstatistieken expliciet als "onbekend, niet gebouwd" gemarkeerd in plaats van weggelaten. Rekenkern (posts/week, regelmaat via variatiecoëfficiënt, engagement-rate) en Gemini Flash-captionclustering gebouwd en getest (execution 97083): rekenkundig correcte uitkomsten op realistische testdata. Actor levert max. 12 posts i.p.v. de gevraagde 25, expliciet gelabeld als beperking. Zie LOG.md "Taak 4.5".
- [x] **Stap 4.6: Screenshots-branch.** Gebouwd met `apify/screenshot-url` (actorId `rGCyoaKTKhyMiiTvS`): desktop (1440px) en mobiel (390px), upload naar de test-Drive-map, resultaat onder sleutel `screenshots`. Laag-3-fallback niet mogelijk met deze actor (levert alleen PNG, geen gerenderde HTML); expliciet gelabeld in `screenshots.laag3_status`, kandidaat-oplossing (Firecrawl) genoemd voor een latere sessie. Visuele-rubric-stub eerlijk bijgewerkt (screenshots wel gemaakt, vision-koppeling bewust niet gebouwd/geverifieerd). Getest (execution 97079), geen node-fouten. Zie LOG.md "Taak 4.6".
...
- [x] **Stap 6.1: Pack-doc.** Gebouwd: één Google Drive-node (`createFromText`, `convertToGoogleDocument: true`, credential "Hidde Drive"), documenttekst volgens `pack-template.md` (titel, partnerregel, datum, workflow-versie, kostenregel, leeg reviewblok, notatieregel, volledig pack-concept, slotregel). De oorspronkelijke Google Docs create+update-nodes zijn vervangen: de Docs-node ondersteunt geen Shared Drives (bekende n8n-bug, community-issue #14997); één Google Drive-node schrijft wel rechtstreeks in de Shared Drive-map. Echt artefact bevestigd in execution 97299.
- [x] **Stap 6.2: Prefill-JSON.** Gebouwd: Convert to File (toJson) plus Google Drive-upload, zelfde patroon als de screenshot-upload uit taak 4.6, met echte datum in `generated_at` (niet meer de placeholder-tekst). Echt bestand bevestigd in execution 97299, correcte bestandsnaam na een bugfix (zie LOG.md).
...
- [x] **Stap 6.4:** Echte end-to-end-run gelukt (execution 97299): echt Google Doc en echt prefill-JSON-bestand aangemaakt in de Shared Drive-testmap, na het oplossen van drie samenhangende problemen (Docs-node ondersteunt geen Shared Drives, Docs API stond uit in het verkeerde Google Cloud-project, twee nieuwe bugs in de eindresultaat-node) en twee nieuwe bugs gevonden en gefixt (`pack_doc_url` wees naar het verkeerde bestand, prefill-bestandsnaam was "undefined"). Volledige toedracht in LOG.md. Handmatig openen en beoordelen van doc/JSON blijft Hidde's eigen actie.

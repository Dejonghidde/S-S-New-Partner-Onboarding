...
## Taak 6: Assemblage en output (2026-07-16)

Op instructie Hidde ("ga door met stap 6!") direct doorgebouwd na taak 4.4/4.5/4.6.

**Bouw:** negen nodes na "Taak 5 afronden": "Kostenregel samenstellen" (Code, zie hieronder), "Documenttekst samenstellen" (Code, bouwt de volledige documenttekst volgens `pack-template.md`: titel, partnerregel, datum, workflow-versie, kostenregel, leeg reviewblok, de notatieregel-uitleg, het complete `pack_concept` inclusief aannames, en een slotregel met dezelfde datum/versie/kosten), "Pack-doc aanmaken" (Google Docs, resource document/create, in de test-Drive-map, titel `{{bedrijfsnaam}} | Research Pack | S&S (PROEF)`), "Pack-doc tekst invoegen" (Google Docs, resource document/update, voegt de documenttekst in als platte tekst aan het einde van het document), "Prefill-JSON voorbereiden" (Code, zet `generated_at` op de echte datum in plaats van de placeholder-tekst uit taak 5), "Prefill-JSON naar bestand" (Convert to File, operatie toJson), "Prefill-JSON uploaden naar Drive" (Google Drive upload, zelfde patroon en credential als de screenshot-upload uit taak 4.6, bestandsnaam `{{bedrijfsnaam}}-prefill-v1.json`), een Merge (zelfde `includeUnpaired: true`-veiligheidspatroon) en "Taak 6 afronden" die de doc-URL, de prefill-JSON-URL en de kostenregel samenvat in het einde-resultaat.

**Credential gevonden voor de Google Docs-node:** het openstaande punt uit de actielijst ("welke credential dekt de Docs-node") is opgelost: `list_credentials` toont een credential van het exacte type `googleDocsOAuth2Api`, genaamd "Google - Accounts@sprintsandsneakers.com". Deze is gebruikt voor zowel "Pack-doc aanmaken" als "Pack-doc tekst invoegen" (automatisch herkend bij het bijwerken van de workflow).

**Kostenregel (stap 6.3), eigen invulling met echte, opgezochte tarieven:** in plaats van een slag in de lucht is voor elke Apify-actor die deze workflow gebruikt het actuele, gepubliceerde FREE-tier-tarief opgehaald via Apify's Store API (`GET /v2/acts/{actorId}`, veld `pricingInfos`, per 2026-07-16): Google Maps zoeken (place-scraped $0,005 + start $0,00005), Google reviews ($0,0006/review + start), Meta-ads ($0,00075/ad), Google-ads ($0,0015/result + start), LinkedIn (start $0,005 + company-scraped $0,00345), Instagram ($0,0026/profiel). De kostenregel berekent deze op basis van de daadwerkelijk gevonden aantallen in de run (niet de aangevraagde maximum), converteert naar euro (indicatieve koers) en somt op tot een Apify-deeltotaal. Expliciet, eerlijk uitgesloten uit het eurototaal (met reden erbij): Semrush (unit-verbruik wel getoond, euro-omrekening onbekend zonder toegang tot het S&S-factuurdashboard), screenshots (geen vast PPE-tarief, standaard Apify-computekosten), en LLM-tokens (geen tokenaantal apart gelogd). Dit is bewust een gedeeltelijke, gelabelde schatting in plaats van een vals-precies totaalbedrag.

**Gevonden en gecorrigeerde fout (uit taak 5, ontdekt tijdens het testen van taak 6):** `openstaande_twijfels_crosscheck` in "Taak 5 afronden" kwam altijd leeg binnen. Oorzaak: de node las `$input.first().json.output`, ervan uitgaand dat de Cross-check-agent zijn directe voorganger was, maar de werkelijke ketting loopt via `Cross-check → Prefill-extractie → Taak 5 afronden`, dus `$input` was steeds `Prefill-extractie`'s resultaat (dat geen `output`-veld heeft). Opgelost door expliciet `$('Cross-check').first().json.output` te lezen. Bevestigd met een echte testrun (execution 97232): het veld bevat nu de volledige, inhoudelijk sterke factcheck-tekst van de cross-check-agent (onder andere een terechte correctie op het te stellige "TikTok niet actief"-label en de "reputatie uitsluitend positief"-conclusie).

**Test 1 (execution 97226, volledig gepind inclusief Pack-doc/Drive-nodes):** structurele validatie van de hele taak-6-keten. Kostenregel correct berekend (€0,0181 Apify-indicatie met volledige uitsplitsing per branch), Documenttekst correct samengesteld (header, reviewblok, notatieregel, pack-concept, slotregel), Prefill-JSON correct met echte datum (`2026-07-16` in plaats van de placeholder), Taak 6-eindresultaat bevat alle verwachte velden (`pack_doc_url`, `prefill_json_url`, `prefill_json_naam`, `kosten_regel`). Geen node-fouten.

**Test 2 (execution 97232, na de crossCheckOutput-fix, Pack-doc/Drive-nodes bewust NIET gepind om een echt artefact te maken):** de volledige pipeline liep opnieuw correct door (inclusief zeven echte, live LLM-aanroepen en de bevestigde crossCheckOutput-fix), maar strandde op "Pack-doc aanmaken" met een 404 "File not found" op de folder-ID van de test-Drive-map. Dit is geen bug in de workflow: de Google Docs-credential "Google - Accounts@sprintsandsneakers.com" heeft kennelijk geen toegang tot die specifieke Drive-map. Geen verdere pogingen met andere credentials gedaan (geen giswerk op een live Drive-map); dit staat als concreet actiepunt op de actielijst voor Hidde.

**Openstaand (blokkade voor stap 6.4, geen blokkade voor de rest):** Hidde moet ofwel de test-Drive-map ("Gamma - Hidde test map") delen met het account achter de credential "Google - Accounts@sprintsandsneakers.com", ofwel aangeven welke credential/account wel toegang heeft tot die map, voordat er een echt research-pack-document aangemaakt kan worden. Zodra dat opgelost is, kan stap 6.4 (een echte end-to-end-run met handmatige controle van het document en het JSON-bestand) alsnog gedraaid worden.

**Bewuste scope-keuze:** de documenttekst wordt als platte tekst ingevoegd (geen opgemaakte koppen/vet via de Google Docs API se text-style-requests); dit is voldoende voor een proefpack en voorkomt onnodige complexiteit in de eerste versie. De cross-check-bevindingen (`openstaande_twijfels_crosscheck`) worden nog steeds niet automatisch in het document verwerkt, alleen los doorgegeven in het workflow-resultaat; dit was al in taak 5 als bewuste, latere stap gemarkeerd.

**Kosten:** twee testruns met elk zeven echte LLM-aanroepen (reviews-thema, captions-clustering, profiel, markt, funnel, aannames-synthese, cross-check); verwaarloosbaar (enkele centen per run). Geen Apify-kosten in test 1 (gepind); test 2 gebruikte dezelfde gepinde Apify-responses voor de deterministische branches, dus ook daar geen echte Apify-kosten. Geen Google Docs/Drive-kosten (Google-API's zijn gratis; het mislukte schrijfverzoek in test 2 kostte niets).

---

## Taak 6 vervolg: stap 6.4 opgelost (2026-07-16)

De 404 uit test 2 bleek geen simpel toegangsprobleem, maar drie samenhangende, opeenvolgende problemen. Elk is met echte diagnose gevonden, niet met gokken op de live map.

**Root cause 1: de Google Docs-node ondersteunt geen Shared Drives (bekende n8n-bug).** De map "Gamma - Hidde test map" zit in een Shared Drive (root-ID `0AMl3InXspt_qUk9PVA`, vastgesteld door de map-metadata via de Google Drive-connector omhoog te volgen tot een folder zonder `parentId`), niet in iemands "Mijn Drive". Onderzoek (n8n community-forum, GitHub issue #14997) bevestigt: de Google Docs-node stuurt bij het aanmaken van een document in een map geen `supportsAllDrives=true` mee, waardoor elke Shared Drive-map altijd een 404 geeft, ongeacht rechten. Eerste reparatiepoging (document aanmaken in "Mijn Drive" van de Docs-credential, dan verplaatsen met een Google Drive-node) liep vast op een tweede probleem: de verplaats-stap gebruikte een andere Google-identiteit (credential "S&S N8N - Drive API", later "Hidde Drive") dan de credential die het document had aangemaakt, en een privé "Mijn Drive"-bestand is voor andere identiteiten onzichtbaar totdat het gedeeld is.

**Echte oplossing: één Google Drive-node in plaats van twee Google Docs-nodes.** "Pack-doc aanmaken" is herbouwd rond `n8n-nodes-base.googleDrive`, resource `file`, operatie `createFromText` (niet `document/create`): deze operatie zet tekst rechtstreeks om in een native Google Doc (optie `convertToGoogleDocument: true`) en kan, in tegenstelling tot de Docs-node, wél rechtstreeks in een Shared Drive-map schrijven (bevestigd doordat de al langer werkende screenshot- en prefill-JSON-uploads dezelfde node-familie gebruiken). Dit vervangt de oude drie-stappen-keten (Docs aanmaken → verplaatsen → Docs tekst invoegen) door één node; "Pack-doc tekst invoegen" en de verplaats-stap zijn overbodig en verwijderd.

**Root cause 2 (bijkomend): Google Docs API stond uit in het onderliggende Google Cloud-project.** `convertToGoogleDocument` roept de Docs API aan; met de credential "S&S N8N - Drive API" gaf dit een 403 ("Google Docs API has not been used in project 230229832509"). Hidde heeft de API aangezet, maar in een ánder Google Cloud-project dan waar deze credential bij hoort; overgestapt op een nieuwe, door Hidde aangemaakte credential "Hidde Drive" (wél met de Docs API aan) loste dit direct op.

**Test 3 (execution 97296, eerste echte geslaagde end-to-end-run):** een echt Google Doc en een echt prefill-JSON-bestand zijn aangemaakt in de Shared Drive-testmap. Alle zeven AI-agents leverden inhoudelijk sterke, correct brongelabelde Nederlandse tekst. Twee nieuwe bugs zichtbaar in de output: `pack_doc_url` verwees naar het prefill-bestand in plaats van het Doc (de Merge-node overschreef het `id`-veld van de ene tak met dat van de andere, omdat beide takken een veld met dezelfde naam hadden), en de prefill-JSON heette "undefined-prefill-v1.json" ("Prefill-JSON uploaden naar Drive" las `$json.partner` van zijn onmiddellijke voorganger, die dat veld niet meer meegeeft na de bestandsconversie).

**Bugfixes:** "Taak 6 afronden" leest nu expliciet `$('Pack-doc aanmaken').first().json` en `$('Prefill-JSON uploaden naar Drive').first().json` in plaats van het samengevoegde Merge-resultaat (voorkomt veldnaam-botsingen structureel, niet alleen voor dit geval). De bestandsnaam van de prefill-upload verwijst nu naar `$('Prefill-JSON voorbereiden').item.json.partner` (dezelfde referentie-discipline als de eerdere crossCheckOutput-fix uit taak 5).

**Test 4 (execution 97299, bevestiging na de fixes):** `pack_doc_url` (`.../document/d/1Mmrsgk9fBMc7MZfm0vlVSdeLqwsj5-h5AGNbDTZdoGE/edit`) en `prefill_json_url` (`.../file/d/1TF5AsyPgJTBILFWDUPX4nI0_ijVcRgFN/...`) wijzen nu naar twee verschillende, correcte bestanden; `prefill_json_naam` is correct "Mollie-prefill-v1.json". Stap 6.4 is hiermee inhoudelijk afgerond; het handmatig openen en beoordelen van het document en het JSON-bestand blijft Hidde's eigen actie.

**Partner-matching-vraag (Hidde, tijdens dit werk):** hoe de vragenlijst straks weet welke prefill-JSON bij welke partner hoort, is onderzocht en blijkt al ontworpen, niet vergeten. De sleutel die het hele systeem verbindt is de Drive-map-id van de partner (aangemaakt door Make-scenario 6525431, vastgelegd in de registry-tab, letterlijk hergebruikt als `?c=`-token door de Cloudflare-vragenlijst). `Uitwerking-Fase-2-Vragenlijst-en-Research-Pack.md` beschrijft al hoe de "verificatiekaart" die token gaat hergebruiken. Deze workflow (ticket 16, "Fundament") identificeert een partner bewust nog via bedrijfsnaam-tekst en schrijft naar de vaste testmap; het omzetten naar de echte per-partner-folder-id hoort bij ticket 17 ("Research Pack Automatisering", gated), niet bij dit ticket.

**Kosten:** twee extra echte end-to-end-testruns (elk zeven LLM-aanroepen, verwaarloosbaar) plus twee echte Google Doc- en twee echte prefill-JSON-bestanden in de Shared Drive-testmap (geen kostenpost, Google Drive/Docs-API's zijn gratis). Er staan hierdoor inmiddels een paar "(PROEF)"-testdocumenten in "Gamma - Hidde test map"; opschonen kan wanneer gewenst, geen actie ondernomen zonder te vragen.
